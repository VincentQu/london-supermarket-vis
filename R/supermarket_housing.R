...
### Mapping London boroughs coloured by their average house price
avgHousePrices <- housing13 %>%
  group_by(region) %>%
  summarise(meanPrice = mean(price)) %>%
  mutate(region = factor(gsub(" AND ", " & ", region)),
         region = factor(gsub("CITY OF WESTMINSTER", "WESTMINSTER", region))) %>%
  arrange(desc(meanPrice))
avgHousePrices

boroughs <- mutate(boroughs, BOROUGH = factor(toupper(BOROUGH)))
boroughs <- merge(boroughs, avgHousePrices, by.x = "BOROUGH", by.y = "region")

ggplot() +
  # Boroughs
  geom_polygon(data = boroughs, aes(x = long, y = lat, group = group, fill = meanPrice)) +
  # Stores
  geom_point(data = filter(supermarkets, Retailer == "Waitrose"),
             aes(x = Lon, y = Lat), 
             colour = "#6AA628") +
